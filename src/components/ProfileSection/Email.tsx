...
const Email: React.FC = () => {
  ...
  const handleSubmit = ()=>{
    ...
    if(userId){
      dispatch(changeEmail({email:newEmail,userId:userId}));
      setVisible(true)
    }
  }
  return (
  ...
  <div className="m-3 w-full rounded-lg p-6 ">
    ...
    <p className="text-sm text-gray-400 mb-4">
      Current email
      <span className="block mt-1 text-white">Your current email address is {email}</span>
    </p>
    ...
    <div className="p-4 mb-4 bg-yellow-700 rounded-lg">
      <p className="text-sm font-semibold text-white">⚠️ Connected account</p>
      <p className="text-sm text-white">
        Your account is connected to a Google account. Changing the email address here will disconnect
    ...
    <div className="flex flex-col mb-4">
      ...
      />
      <button className="w-24 px-4 py-2 text-white bg-zinc-800 rounded" onClick={handleSubmit}>
        Save
      </button>
    </div>
